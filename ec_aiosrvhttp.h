...
\update 
  2024-2-1   use ec::string::appendformat instead of ec::array<char>
		...
		protected:
			...
			*/
			...
			bool DoHead(int fd, const char* sfile, ec::http::package* pPkg)
			{
				...
				answer.appendformat("Content-Length: %lld\r\n\r\n", flen);
				if (this->_plog)
			...
			bool downfile(int fd, ec::http::package* pPkg, const char* sfile)
			{
				...
				EC_STACKSTRING(content_type, 256);
				bool bzip = true;
				if (sext && *sext) {
			...
			bool downbigfile(int fd, http::package* pPkg, const char* sfile, long long filelen)
			{
				...
				data.appendformat("Content-Length: %lld\r\n\r\n", filelen);
			...
			bool DoGetRang(int fd, const char* sfile, ec::http::package* pPkg, int64_t lpos, int64_t lposend, int64_t lfilesize)
			{
				...
				answer.appendformat("Content-Range: bytes %jd-%jd/%jd\r\n", lpos, (lpos + sizeContent - 1), lfilesize);
				answer.appendformat("Content-Length: %jd\r\n\r\n", sizeContent);
			...
			bool dohttp(int fd, const uint8_t* pkg, size_t pkgsize)
			{
				...
				EC_STACKSTRING(sfile, 1024);
				EC_STACKSTRING(utf8, 1024);
				EC_STACKSTRING(pathmaproot, 1024);
