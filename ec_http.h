...
\update
  2024.11.9 remove inlcude ec_array.h
...
namespace ec
{
	...
	*/
	...
	namespace http
	{
		...
		{
			const char* s[] = { ".zip", ".rar", ".tar", ".7z", ".gz", ".jpg", ".jpeg", ".gif", ".arj", ".jar", ".cab", ".bz2"};
			size_t i = 0;
		...
		*/
		...
		public:
			...
			ec::vector<t_i> _head;//head items
			ctxt _body; // body
		public:
			inline void clear()
			{
			...
			{
				pout->clear();
				pout->appendformat("HTTP/1.1 %d %s\r\n", statuscode, statusmsg);
				if (HasKeepAlive())
				...
				if (Content_type && *Content_type) {
					pout->appendformat("Content-type: %s\r\n", Content_type);
				}
				else
					pout->append("Content-type: application/octet-stream\r\n");

				int bdeflate = 0;
				EC_STACKSTRING(stmp, 1024);
				...
				pout->appendformat("Content-Length: %9d\r\n\r\n", (int)bodysize);
				sizehead = pout->size();
				if (bdeflate) {
